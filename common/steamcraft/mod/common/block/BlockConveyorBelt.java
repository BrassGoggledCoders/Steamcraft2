 ...
 *
 */
public class BlockConveyorBelt extends BlockSC2 {
	/** */
	...
	public BlockConveyorBelt(int id) {
		...
		this.setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, ConveyorHelper.HEIGHT, 1.0F);
		//this.timePeriod = 0;
	...
	public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity) {
		if (ConveyorHelper.isEntityIgnored(entity)) {
			return;
		}
		if (entity instanceof EntityItem) {
			ConveyorHelper.packItems(world, i, j, k);
		}
		if (entity instanceof EntityItem && !this.hasSpeedBoost) {
			ConveyorHelper.doSpecialItemAction(world, i, j, k, (EntityItem) entity);

			if (ConveyorHelper.storeNearby(world, i, j, k, (EntityItem) entity, false)) {
				return;
			}
		}

		int direction = ConveyorHelper.getRotation(world.getBlockMetadata(i, j, k));
		...
		boolean leadsToNowhere = ConveyorHelper.isObstructed(world, i, j, k);
		leadsToNowhere = leadsToNowhere && ConveyorHelper.isItemBeyondStorageBorder(world, direction, i, j, k, entity, ConveyorHelper.STORAGE_BORDER_LONG);

		if (!leadsToNowhere) {
			ConveyorHelper.entityPreventDespawning(world, i, j, k, true, entity);
		}

		double speed_max = ConveyorHelper.MAX_HORIZONTAL_SPEED;

		if (this.hasSpeedBoost) {
			speed_max *= 2.0D;
		}

		double boost = ConveyorHelper.HORIZONTAL_BOOST;

		if (this.hasSpeedBoost) {
			boost *= 2.0D;
		}

		ConveyorHelper.moveEntityOnBelt(world, i, j, k, entity, true, !leadsToNowhere, direction, speed_max, boost);
	}

	@Override
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int i, int j, int k) {
		return AxisAlignedBB.getBoundingBox(i, 0.0F + j, k, (i + 1), (j + ConveyorHelper.HEIGHT_COLLISION + 0.0F), (k + 1));
	}
	...
	public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int i, int j, int k) {
		float f = 0;
		f = 0.0F + ConveyorHelper.HEIGHT_SELECTED;
		return AxisAlignedBB.getBoundingBox(i, 0.0F + j, k, (i + 1), j + f, (float) k + 1);
	}

	@Override
	public void setBlockBoundsBasedOnState(IBlockAccess iblockaccess, int i, int j, int k) {
		this.setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 0.0F + ConveyorHelper.HEIGHT, 1.0F);
	}
