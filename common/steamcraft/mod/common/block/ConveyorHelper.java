...
public class ConveyorHelper {
	...
	public static boolean storeEntityItemAt(World world, int i, int j, int k, EntityItem entity) {
		IInventory inventory = SC2_InventoryUtils.getCompositeInventoryAt(world, i, j, k);

		if (inventory != null && entity != null && entity.isEntityAlive()) {
			ItemStack stackToStore = entity.getEntityItem();

			if (stackToStore != null && SC2_InventoryUtils.storeItemInInventory(inventory, stackToStore)) {
	...
	public static void doSpecialItemAction(World world, int i, int j, int k, EntityItem entityItem) {
		...
		do {
			if (ConveyorHelper.performSpecialItemActions(world, i, j, k + 1, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i, j, k - 1, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i + 1, j, k, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i - 1, j, k, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i, j - 1, k + 1, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i, j - 1, k - 1, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i + 1, j - 1, k, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i - 1, j - 1, k, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i, j + 1, k, entityItem)) {
				break;
			}
			if (ConveyorHelper.performSpecialItemActions(world, i, j - 1, k, entityItem)) {
				break;
	...
	public static void moveEntityOnBelt(World world, int i, int j, int k, Entity entity, boolean bordersEnabled, boolean motionEnabled, int moveDirection, double max_horizontal_speed, double horizontal_boost) {
		if (motionEnabled && world.rand.nextInt(35) == 0) {
			...
			if (world.rand.nextInt(list.size() + 1) == 0) {
				ConveyorHelper.playSFXBelt(world, i, j, k);
			}
		}
