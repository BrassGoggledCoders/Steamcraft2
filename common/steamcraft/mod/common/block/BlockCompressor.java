...
public class BlockCompressor extends BlockSC2Container
{
	...
	{
		blockIcon = par1IconRegister.registerIcon(SC2_Info.SC2_PREFIX + "compressorside");
		iconFront = par1IconRegister.registerIcon(SC2_Info.SC2_PREFIX + "compressorfrontinactive");
		iconFrontA = par1IconRegister.registerIcon(SC2_Info.SC2_PREFIX + "compressorfrontinactive");
		iconTop = par1IconRegister.registerIcon(SC2_Info.SC2_PREFIX + "compressortop");
	}
			...
			float par9)
	{
		...
		else
		{
			...
			if (tile_entity == null || par5EntityPlayer.isSneaking())
				return false;
			par5EntityPlayer.openGui(SC2.instance, SC2_GuiIDs.GUI_ID_COMPRESSOR, par1World, par2, par3, par4);
			return true;
		}
	}
