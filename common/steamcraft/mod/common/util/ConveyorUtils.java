...
public class ConveyorUtils {
	...
	public static boolean storeEntityItemAt(World world, int i, int j, int k, EntityItem entity) {
		IInventory inventory = InventoryUtils.getCompositeInventoryAt(world, i, j, k);

		if (inventory != null && entity != null && entity.isEntityAlive()) {
			ItemStack stackToStore = entity.getEntityItem();

			if (stackToStore != null && InventoryUtils.storeItemInInventory(inventory, stackToStore)) {
	...
	public static void doSpecialItemAction(World world, int i, int j, int k, EntityItem entityItem) {
		...
		do {
			if (ConveyorUtils.performSpecialItemActions(world, i, j, k + 1, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i, j, k - 1, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i + 1, j, k, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i - 1, j, k, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i, j - 1, k + 1, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i, j - 1, k - 1, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i + 1, j - 1, k, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i - 1, j - 1, k, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i, j + 1, k, entityItem)) {
				break;
			}
			if (ConveyorUtils.performSpecialItemActions(world, i, j - 1, k, entityItem)) {
				break;
	...
	public static void moveEntityOnBelt(World world, int i, int j, int k, Entity entity, boolean bordersEnabled, boolean motionEnabled, int moveDirection, double max_horizontal_speed, double horizontal_boost) {
		if (motionEnabled && world.rand.nextInt(35) == 0) {
			...
			if (world.rand.nextInt(list.size() + 1) == 0) {
				ConveyorUtils.playSFXBelt(world, i, j, k);
			}
		}
