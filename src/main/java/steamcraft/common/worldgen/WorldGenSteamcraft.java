...
public class WorldGenSteamcraft implements IWorldGenerator
{
	...
	private void generateSurface(World world, Random random, int chunkX, int chunkZ)
	{
		...
		/*
		 * for(int i = 0; i < 1; i++) { int X = chunkX + random.nextInt(16); int Z = chunkZ + random.nextInt(16); int Y = random.nextInt(6); new
		 * WorldGenFissurePortal().generate(world, random, X, Y, Z); }
		 */
	}
