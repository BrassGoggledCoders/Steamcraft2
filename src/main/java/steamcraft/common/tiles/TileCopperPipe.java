...
public class TileCopperPipe extends TileEntity
{
		...
		if (tile instanceof TileCopperPipe)
		{
			...
			if (pipe.network != null)// Should only happen on world load
			{
				if (!pipe.network.equals(network))
				{
					...
					else if ((network.tank.getFluid() == null) || (pipe.network.tank.getFluid() == null))
					{
			...
			if (network.size != 0)
			{
				for (ForgeDirection dir : connections)
					if (dir != null)
						if (isCopperPipe(dir))
						{
							TileCopperPipe pipe = (TileCopperPipe) worldObj.getTileEntity(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord
									+ dir.offsetZ);
							...
							if (network != null)
							{
								...
								if (network.tank.getFluid() != null)
									pipe.network.tank.setFluid(new FluidStack(network.tank.getFluid(), network.tank.getFluidAmount()
	...
	public static class FluidNetwork
	{
		...
		public FluidNetwork(int size)
		{
			...
			tank = new FluidTank(capacityPerPipe * size);
		}
		...
		private void updateClient(TileCopperPipe pipe)
		{
			...
			{
				fluidScaled = (tank.getFluidAmount() / (float) size / FluidNetwork.capacityPerPipe) * (2 * TileCopperPipeRenderer.pixel);
				...
				InitPackets.network.sendToAllAround(new FluidNetworkPacket(pipe.worldObj.provider.dimensionId, pipe.xCoord, pipe.yCoord, pipe.zCoord,
						fluidScaled, tank.getFluid().getFluid().getName()), new TargetPoint(pipe.worldObj.provider.dimensionId, pipe.xCoord,
						pipe.yCoord, pipe.zCoord, 100));
			}
			else
				InitPackets.network.sendToAllAround(new FluidNetworkPacket(pipe.worldObj.provider.dimensionId, pipe.xCoord, pipe.yCoord, pipe.zCoord,
						0, "water"), new TargetPoint(pipe.worldObj.provider.dimensionId, pipe.xCoord, pipe.yCoord, pipe.zCoord, 100));
		}
