...
public class TileSteamBoiler extends TileEntityMachine implements IFluidHandler {
	...
	public TileSteamBoiler() {
		...
		this.waterTank = new FluidTank(new FluidStack(FluidRegistry.WATER, 0),
				5000);
		this.steamTank = new FluidTank(new FluidStack(
	...
	public void readFromNBT(NBTTagCompound tag) {
		...
		this.steamTank.setFluid(new FluidStack(FluidRegistry.getFluid("steam"),
				tag.getShort("steamLevel")));
		...
		if (!this.worldObj.isRemote) {
			if (this.inventory[1] != null) {
				FluidStack liquid = FluidContainerRegistry
						.getFluidForFilledItem(this.inventory[1]);
				...
				if ((liquid != null)
					...
					if (liquid.getFluid() == FluidRegistry.WATER) {
						...
						if (this.inventory[1].stackSize > 1) {
							...
							this.inventory[1] = this.inventory[1].getItem()
									.getContainerItem(this.inventory[1]);
						}
					}
				}
			}
			if (inventory[2] != null
					&& inventory[2].getItem() instanceof ItemCanister) {
				ItemCanister canister = (ItemCanister) inventory[2].getItem();
				if (steamTank.getFluidAmount() > 0
						&& canister.getFluidAmount(inventory[2]) != ItemCanister.MAX_STEAM) {
					canister.fill(inventory[2],
			...
			if ((this.getItemBurnTime() > 0)
					&& (this.furnaceBurnTime == 0)
					&& (this.waterTank.getFluidAmount() >= waterPerTick)
					&& (this.steamTank.fill(
							new FluidStack(FluidRegistry.getFluid("steam"),
				...
				if (this.inventory[0].stackSize == 1) {
					this.inventory[0] = this.inventory[0].getItem()
							.getContainerItem(this.inventory[0]);
				} else {
			...
			if ((this.furnaceBurnTime > 0)
					&& (this.waterTank.getFluidAmount() >= waterPerTick)
					&& (this.steamTank.getFluidAmount() <= 10000)) {
				this.steamTank.fill(
						new FluidStack(FluidRegistry.getFluid("steam"),
								steamPerTick), true);
			...
			if (var1 != (this.furnaceBurnTime > 0)) {
				var2 = true;
				BlockSteamBoiler.updateFurnaceBlockState(
	...
	public int getScaledWaterLevel(int i) {
		return this.waterTank.getFluid() != null ? (int) (((float) this.waterTank
				.getFluid().amount / (float) (this.waterTank.getCapacity())) * i)
				: 0;
	}
	...
	public int getScaledSteamLevel(int i) {
		return this.steamTank.getFluid() != null ? (int) (((float) this.steamTank
				.getFluid().amount / (float) (this.steamTank.getCapacity())) * i)
				: 0;
	}
	...
	public FluidStack drain(ForgeDirection from, FluidStack resource,
			boolean doDrain) {
		if ((resource == null)
				|| !resource.isFluidEqual(this.steamTank.getFluid())) {
			return null;
		}
