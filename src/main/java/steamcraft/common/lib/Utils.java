...
public class Utils
{
	...
	private void spawnParticles(Coord cnt, Coord move, World world, int half, double color_r, double color_g, double color_b)
	{
		Random rand = new Random();
		double shift = 0;
		if (half == 0) shift = -0.5D + rand.nextFloat() * 1.0D;
		if (half == 1) shift = -rand.nextFloat() * 0.5D;
		if (half == 2) shift = rand.nextFloat() * 0.5D;

		double d1o = cnt.x + 0.5D;
		double d2o = cnt.y + 0.5D;
		double d3o = cnt.z + 0.5D;
		...
		if (move.x > 0) d1 = d1o + shift;
		if (move.y > 0) d2 = d2o + shift;
		if (move.z > 0) d3 = d3o + shift;

		if (move.x < 0) d1 = d1o - shift;
		if (move.y < 0) d2 = d2o - shift;
		if (move.z < 0) d3 = d3o - shift;

		FMLClientHandler.instance().getClient().effectRenderer.addEffect(new FXLaser(world, d1, d2, d3, (float) color_r, (float) color_g, (float) color_b, move.x, move.y, move.z, half));
	}
}
