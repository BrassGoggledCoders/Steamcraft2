...
public class EventHandlerDrawHighlight {
	...
	public void renderOverlay(RenderGameOverlayEvent.Text event) {
		...
		fontRenderer.drawString("Block: " + this.block.getUnlocalizedName(),
				posX, posY2, color);
	}
	...
	public void onDrawBlockSelectionBox(DrawBlockHighlightEvent event) {
		if ((event.player.inventory.armorItemInSlot(3) != null)
			...
			this.drawSelectionBox(event.player, event.target, 0,
		...
		this.block = event.player.worldObj.getBlock(event.target.blockX,
	 ...
	 *            the partial ticks
	 */
	private void drawSelectionBox(EntityPlayer player,
			MovingObjectPosition mop, int i, ItemStack is, float partialTicks) {
		if ((i == 0) && (mop.typeOfHit == MovingObjectType.BLOCK)) {
			...
			float offset = 0.002F;
			Block block = player.worldObj.getBlock(mop.blockX, mop.blockY,
					mop.blockZ);
			...
			if (block != Blocks.air) {
				...
				this.drawOutlinedBoundingBox(block
		...
		if ((i == 0) && (mop.typeOfHit == MovingObjectType.ENTITY)) {
			...
			if (entity != null) {
				...
				double dz = player.lastTickPosZ
						+ ((player.posZ - player.lastTickPosZ) * partialTicks);
				this.drawOutlinedBoundingBox(entity.boundingBox.expand(offset,
