...
{// implements IPacketHandler {
	 ...
	 * PACKET_CLIENT_UPDATE: handleUpdatePacket("client", dataStream, player);
	 * break; case PACKET_GUI_INFO: handleGUIInfoPacket(dataStream); break; case
	 * PACKET_OPEN_SERVER_GUI: handleOpenServerGui(packet, (EntityPlayer)
	 * player, dataStream); break; case PACKET_EXTENDED_PROPERTIES:
	 * handleExtendedProperties(packet, player, dataStream); default:
	...
	private void handleGUIInfoPacket(final DataInputStream dataStream)
			throws IOException
	{
		final int windowID = dataStream.readByte();
		final int barID = dataStream.readShort();
		final int content = dataStream.readInt();

		final net.minecraft.inventory.Container container = Minecraft
