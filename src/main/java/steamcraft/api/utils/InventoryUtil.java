 ...
 * 
 * File created @ [Mar 14, 2014, 10:37:19 AM]
 */
...
public class InventoryUtil
{
	...
	/** Stores Item into a specified slot in an inventory. */
	public static boolean storeItemInSlot(IInventory inventory, ItemStack stack, int slot) 
	{
		...
		if (Item == null) 
		{
			int storedItem = stack.stackSize;
			storedItem = Math.min(storedItem, stack.getMaxStackSize());
			storedItem = Math.min(storedItem, inventory.getInventoryStackLimit());
			Item = stack.splitStack(storedItem);
			inventory.setInventorySlotContents(slot, Item);
			return true;
		}
		if (Item.getItem() == stack.getItem() && Item.isStackable() && (!Item.getHasSubtypes() || Item.getItemDamage() == stack.getItemDamage()) && Item.stackSize < inventory.getInventoryStackLimit()) 
		{
			int storedItem = stack.stackSize;
			storedItem = Math.min(storedItem, Item.getMaxStackSize() - Item.stackSize);
			storedItem = Math.min(storedItem, inventory.getInventoryStackLimit() - Item.stackSize);
			Item.stackSize += storedItem;
			stack.stackSize -= storedItem;
			return (storedItem > 0);
		}
	...
	public static boolean addItemStackToInventory(IInventory inventory, ItemStack stack) 
	{
		if (!stack.isItemDamaged()) 
		{
			...
			do {
				storedItem = stack.stackSize;
				stack.stackSize = storePartialItemStack(inventory, stack);
			} while (stack.stackSize > 0 && stack.stackSize < storedItem);

			return stack.stackSize < storedItem;
		}
	...
	private static int getStackWithFreeSpace(IInventory inventory, ItemStack stack)
	{
		for (int slot = 0; slot < inventory.getSizeInventory(); slot++)
		{
			...
			if (stackAt != null && stackAt.getItem() == stack.getItem() && stackAt.isStackable() && stackAt.stackSize < stackAt.getMaxStackSize() && stackAt.stackSize < inventory.getInventoryStackLimit() && (!stackAt.getHasSubtypes() || stackAt.getItemDamage() == stack.getItemDamage())) 
			{
				return slot;
			}
		}
	...
	private static int storePartialItemStack(IInventory inventory, ItemStack stack) 
	{
		...
		if (inventory.getStackInSlot(targetSlot) == null) 
			inventory.setInventorySlotContents(targetSlot, new ItemStack(item, 0, stack.getItemDamage()));
	...
	public static boolean isFuel(ItemStack stack) 
	{
		...
		Item item = stack.getItem(); // TODO

		return (item == Item.stick) || (item == Item.coal) || (item == Item.bucketLava);
	}

	public static boolean isSmeltable(ItemStack is) 
	{
		if (is == null || FurnaceRecipes.smelting().getSmeltingResult(is) == null) 
			return false;
	...
	public static int getFuelValue(ItemStack stack, double value) 
	{
		...
		//if (item == Item.getItemFromBlock(Block.planks)) planks
		...
		//if (item == ) sapling
		//	return (int) (100 * value);
		if (item == Item.gunpowder)
			return (int) (500 * value);
		if (item == Item.blazeRod)
	...
	/** Drops inventory contents as Item into the world; called by breakBlock. */ // TODO
	/*
